...
App.views.discover = (function () {
  ...
  function enterSelect() {
    ...
    const wrap = document.getElementById('gfxWallWrap'); if (wrap) wrap.style.display = 'none';
    const meta = document.getElementById('gfxMeta'); if (meta) meta.style.display = 'none';
    updateBatch();
    renderGrid();
  }
  function exitSelect() {
    ...
    const wrap = document.getElementById('gfxWallWrap'); if (wrap) wrap.style.display = '';
    const meta = document.getElementById('gfxMeta'); if (meta) meta.style.display = '';
    renderGrid();
  }
  ...
  // ===== 旋转海报墙（coverflow）：居中高亮，两侧 3D 排开，自动轮播 + 触摸滑动 =====
  let wallIdx = 0, wallTimer = null;

  function paintWall() {
    const box = document.getElementById('gfxWall');
    if (!box) return;
    const movies = state.movies;
    if (!movies.length) { box.innerHTML = '<div class="gfx-ph">🎬</div>'; return; }
    const n = movies.length;
    wallIdx = Math.max(0, Math.min(wallIdx, n - 1));
    const R = 3;
    let html = '';
    for (let o = -R; o <= R; o++) {
      const i = wallIdx + o;
      ...
      const scale = 1 - dist * 0.13;
      const z = 20 - dist * 6;
      html += `<div class="gfx-card ${o === 0 ? 'cur' : ''}" data-tmdb="${m.tmdbId}" style="transform:translateX(${o * 84}px) rotateY(${-o * 24}deg) scale(${scale.toFixed(3)});z-index:${z};opacity:${(1 - dist * 0.22).toFixed(2)}">
        ${m.poster ? `<img src="${App.util.escapeHtml(m.poster)}" onerror="this.parentNode.classList.add('ph');this.remove();" alt="">` : '<div class="gfx-ph">🎬</div>'}
        ${inLibrary(m) ? '<span class="added-badge">已加入</span>' : ''}
      </div>`;
    }
    box.innerHTML = html;
    paintMeta(movies[wallIdx]);
  }
  ...
  function paintMeta(m) {
    ...
    if (!box || !m) return;
    box.innerHTML = `<div class="gfx-title">${App.util.escapeHtml(m.title)}</div>
      <div class="gfx-sub"><span>${m.year || ''}</span>${inLibrary(m) ? '<span class="gfx-added">已加入 ✓</span>' : ''}</div>`;
  }

  function nextWall(dir) {
    const n = state.movies.length;
    if (!n) return;
    wallIdx = Math.max(0, Math.min(wallIdx + dir, n - 1));
    paintWall();
  }
  function resetWall() { wallIdx = 0; }

  function stopWallAuto() { if (wallTimer) { clearInterval(wallTimer); wallTimer = null; } }

  function startWallAuto() {
    stopWallAuto();
    const rm = window.matchMedia && window.matchMedia('(prefers-reduced-motion: reduce)');
    if (rm && rm.matches) return; // 减弱动效时只手动
    wallTimer = setInterval(() => {
      if (!document.getElementById('gfxWall')) { stopWallAuto(); return; } // 离开页面自动停
      const n = state.movies.length;
      if (!n) return;
      wallIdx = wallIdx >= n - 1 ? 0 : wallIdx + 1;
      paintWall();
    }, 3500);
  }

  function bindWallTouch(root) {
    const wall = root.querySelector('#gfxWall');
    if (!wall || wall._bound) return;
    wall._bound = true;
    let tx = null;
    wall.addEventListener('touchstart', e => { tx = e.touches[0].clientX; }, { passive: true });
    wall.addEventListener('touchend', e => {
      if (tx == null) return;
      const dx = e.changedTouches[0].clientX - tx;
      if (Math.abs(dx) > 40) nextWall(dx < 0 ? 1 : -1);
      tx = null;
    }, { passive: true });
    // 点击：中间海报=打开（进详情/快速加入）；两侧海报=跳转到它
    wall.addEventListener('click', e => {
      ...
      if (idx === wallIdx) { openMovie(m); return; }
      wallIdx = idx;
      paintWall();
    });
  }
  ...
  function render(param, root) {
    ...
    root.innerHTML = `
      ...
      <div class="gfx-meta" id="gfxMeta"></div>
